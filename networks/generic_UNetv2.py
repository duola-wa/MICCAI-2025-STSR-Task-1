...
default_dict = {
    ...
    "net_num_pool_op_kernel_sizes": [[1, 1, 1], [2, 2, 2], [2, 2, 2], [2, 2, 2], [2, 2, 2], [2, 1, 2]],
...
def initialize_nnunetv2(threeD=True, num_classes=2):
    ...
    # 修改：去掉 negative_slope，与 plans.json 保持一致
    ...
    kwargs = {
        'PlainConvUNet': {
            ...
            'nonlin': nn.LeakyReLU,
            'nonlin_kwargs': {'inplace': True, 'negative_slope': 0.01},  # 与上面保持一致
        },
        'ResidualEncoderUNet': {
            'conv_bias': True,
            'norm_op': get_matching_instancenorm(conv_op),
    ...
    network = network_class(
        ...
        deep_supervision=False,  # 🚨 关键修改：推理时设为 False
        **conv_or_blocks_per_stage,
